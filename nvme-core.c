...
static struct nvme_iod *nvme_alloc_iod(struct request *rq, struct nvme_dev *dev,
			               gfp_t gfp)
{
	unsigned size = !(rq->cmd_flags & REQ_OP_DISCARD) ? blk_rq_bytes(rq) :
...
static __le64 **phys_iod_list(struct nvme_phys_iod *iod)
{
	return ((void *)iod) + iod->offset;
}

static inline void phys_iod_init(struct nvme_phys_iod *iod, unsigned nbytes,
			    unsigned nseg, unsigned long private)
{
	iod->offset = offsetof(struct nvme_phys_iod, meta_sg[1]);
	iod->npages = -1;
	iod->length = nbytes;
}

static struct nvme_phys_iod *
__nvme_alloc_phys_iod(unsigned nseg, unsigned bytes, struct nvme_dev *dev,
		      unsigned long priv, gfp_t gfp)
{
	struct nvme_phys_iod *iod = kmalloc(sizeof(struct nvme_phys_iod) +
					    sizeof(__le64 *) * nvme_npages(bytes, dev), gfp);

	if (iod)
		phys_iod_init(iod, bytes, nseg, priv);

	return iod;
}

void nvme_free_phys_iod(struct nvme_dev *dev, struct nvme_phys_iod *iod)
{
	const int last_prp = dev->page_size / 8 - 1;
	int i;
	__le64 **list = phys_iod_list(iod);
	dma_addr_t prp_dma = iod->prp2;
								...
								gfp_t gfp)
{
	...
	if (length <= 0) {
		pr_debug("NVMe: nvme_setup_prps() first return, sg = 0x%016llx, dma_addr = 0x%08llx\n", (u64)sg, dma_addr);
		return total_len;
	}
	...
	if (length <= page_size) {
		...
		pr_debug("NVMe: nvme_setup_prps() second return, sg = 0x%016llx, dma_addr = 0x%08llx\n", (u64)sg, dma_addr);
		return total_len;
	}
	...
	if (!prp_list) {
		iod->first_dma = dma_addr;
		iod->npages = -1;
		pr_debug("NVMe: nvme_setup_prps() third return, sg = 0x%016llx, dma_addr = 0x%08llx\n", (u64)sg, dma_addr);
	...
	for (;;) {
		if (i == page_size >> 3) {
			...
			prp_list = dma_pool_alloc(pool, gfp, &prp_dma);
			if (!prp_list) {
				pr_debug("NVMe: nvme_setup_prps() fourth return, sg = 0x%016llx, dma_addr = 0x%08llx\n",
					 (u64) sg, dma_addr);
	...
	pr_debug("NVMe: nvme_setup_prps() final return, sg_dma_address(sg_next(iod->sg)) = 0x%016llx, list[0][0] = 0x%08llx\n", sg_dma_address(sg_next(iod->sg)), list[0][0]);
	return total_len;
}

/* length is in bytes.  gfp flags indicates whether we may sleep. */
int nvme_setup_phys_prps(struct nvme_dev *dev, struct nvme_phys_iod *iod, int total_len,
		    gfp_t gfp, dma_addr_t address)
{
	struct dma_pool *pool;
	int length = total_len;
	int dma_len = total_len;
	u64 dma_addr = address;
	u32 page_size = dev->page_size;
	int offset = dma_addr & (page_size - 1);
	__le64 *prp_list;
	__le64 **list = phys_iod_list(iod);
	...
	if (length <= 0) {
		iod->prp1 = address;
		pr_debug("NVMe: nvme_setup_phys_prps() first return, dma_addr = 0x%08llx\n", dma_addr);
		return total_len;
	}
	...
	if (dma_len) {
		dma_addr += (page_size - offset);
		pr_debug("NVMe: nvme_setup_phys_prps() if(dma_len)\n");
	} else {
		pr_debug("NVMe: nvme_setup_phys_prps() if(!dma_len)\n");
//		sg = sg_next(sg);
//		dma_addr = sg_dma_address(sg);
//		dma_len = sg_dma_len(sg);
	...
	if (length <= page_size) {
		iod->prp1 = address;
		iod->prp2 = address + page_size;
		pr_debug("NVMe: nvme_setup_phys_prps() second return, dma_addr = 0x%08llx\n", dma_addr);
		return total_len;
	}
	...
	if (nprps <= (256 / 8)) {
		pool = dev->prp_small_pool;
		iod->npages = 0;
		pr_debug("NVMe: nvme_setup_phys_prps() using small pool for DMA\n");
	} else {
		pool = dev->prp_page_pool;
		iod->npages = 1;
		pr_debug("NVMe: nvme_setup_phys_prps() using page pool for DMA\n");
	}
	...
	if (!prp_list) {
		...
		iod->prp2 = address + page_size;
		iod->npages = -1;
		pr_debug("NVMe: nvme_setup_phys_prps() third return, dma_addr = 0x%08llx\n", dma_addr);
		return (total_len - length) + page_size;
	}
	list[0] = prp_list;
	iod->prp1 = address;
	iod->prp2 = prp_dma;
	dma_addr = address + page_size;
	i = 0;
	for (;;) {
		if (i == page_size >> 3) {
			__le64 *old_prp_list = prp_list;
			pr_debug("NVMe: nvme_setup_phys_prps() (i == page_size >> 3)\n");
			prp_list = dma_pool_alloc(pool, gfp, &prp_dma);
			if (!prp_list) {
				pr_debug("NVMe: nvme_setup_phys_prps() fourth return, dma_addr = 0x%08llx\n", dma_addr);
				return total_len - length;
			}
			list[iod->npages++] = prp_list;
			prp_list[0] = old_prp_list[i - 1];
	...
	pr_debug("NVMe: nvme_setup_phys_prps() iod->npages = %d\n", iod->npages);
	pr_debug("NVMe: nvme_setup_phys_prps() final return, list[0][0] = 0x%08llx\n", list[0][0]);
	return total_len;
}
							...
							struct nvme_ns *ns)
{
	...
	pr_debug("NVMe: nvme_submit_iod(), qid: %d\n", nvmeq->qid);
			 ...
			 const struct blk_mq_queue_data *bd)
{
	...
	pr_debug("NVMe: nvme_queue_rq(), qid: %d\n", nvmeq->qid);
...
//	pr_debug("NVMe: nvme_process_cq(), qid: %d\n", nvmeq->qid);
...
static irqreturn_t nvme_irq_check(int irq, void *data)
{
	...
	pr_debug("NVMe: nvme_irq_check(), irq: %d\n", irq);
						...
						u32 *result, unsigned timeout)
{
	...
	pr_debug("NVMe: nvme_submit_sync_cmd(), qid: %d\n", nvmeq->qid);
...
static int nvme_submit_async_admin_req(struct nvme_dev *dev)
{
	...
	pr_debug("NVMe: nvme_submit_async_admin_req()\n");

	req = blk_mq_alloc_request(dev->admin_q, WRITE, BLK_MQ_REQ_NOWAIT | BLK_MQ_REQ_RESERVED);
	if (IS_ERR(req))
		return PTR_ERR(req);

	// https://github.com/Xilinx/linux-xlnx/commit/bbc758ec04c2f30805ce0fcdfbaa4c3445fafbae
			...
			struct async_cmd_info *cmdinfo, unsigned timeout)
{
	...
	pr_debug("NVMe: nvme_submit_admin_async_cmd()\n");
...
int nvme_submit_admin_cmd(struct nvme_dev *dev, struct nvme_command *cmd,
								u32 *result)
{
	pr_debug("NVMe: nvme_submit_admin_cmd()\n");
					...
					struct nvme_command *cmd, u32 *result)
{
	...
	req = blk_mq_alloc_request(ns->queue, WRITE, 0);
	if (IS_ERR(req))
		return PTR_ERR(req);
...
static void nvme_abort_req(struct request *req)
{
 ...
 out:
	...
	abort_req = blk_mq_alloc_request(dev->admin_q, WRITE, BLK_MQ_REQ_NOWAIT);
	if (IS_ERR(abort_req))
		return;
...
{
	// This might break something... but the function is missing from newer kernels
//	struct blk_mq_hw_ctx *hctx = nvmeq->hctx;
//
//	spin_lock_irq(&nvmeq->q_lock);
//	if (hctx && hctx->tags)
...
static void nvme_init_queue(struct nvme_queue *nvmeq, u16 qid)
{
	...
	pr_debug("NVMe: nvme_init_queue(), qid: %u\n", qid);
...
static int nvme_create_queue(struct nvme_queue *nvmeq, int qid)
{
	...
	pr_debug("NVMe: nvme_create_queue(), qid: %d\n", qid);
...
{
	pr_debug("NVMe: nvme_disable_ctrl()\n");
...
{
	pr_debug("NVMe: nvme_enable_ctrl()\n");
...
static int nvme_shutdown_ctrl(struct nvme_dev *dev)
{
	...
	pr_debug("NVMe: nvme_shutdown_ctrl()\n");
...
{
	pr_debug("NVMe: nvme_dev_remove_admin()\n");
...
{
	pr_debug("NVMe: nvme_alloc_admin_tags()\n");
...
static int nvme_configure_admin_queue(struct nvme_dev *dev)
{
	...
	pr_debug("NVMe: nvme_configure_admin_queue()\n");
				...
				unsigned long addr, unsigned length)
{
	...
	pr_debug("NVMe: nvme_map_user_pages(), addr: 0x%08lx, length: %u\n", addr, length);
	...
	if (err < count) {
		pr_debug("NVMe: nvme_map_user_pages() get_user_pages_fast() failed (count: %d, err: %d)\n", count, err);
		count = err;
		err = -EFAULT;
		goto put_pages;
	}

	pr_debug("NVMe: nvme_map_user_pages() get_user_pages_fast() success (count: %d)\n", count);
			...
			struct nvme_iod *iod)
{
	...
	pr_debug("NVMe: nvme_unmap_user_pages()\n");
				     ...
				     unsigned long addr, unsigned length)
{
	...
	pr_debug("NVMe: nvme_map_kernel_pages(), addr: 0x%08lx, length: %u\n", addr, length);
...
void nvme_unmap_kernel_pages(struct nvme_dev *dev, int write,
			   struct nvme_iod *iod)
{
	pr_debug("NVMe: nvme_unmap_kernel_pages()\n");

	dma_unmap_sg(&dev->pci_dev->dev, iod->sg, iod->nents,
		     write ? DMA_TO_DEVICE : DMA_FROM_DEVICE);
}

struct nvme_phys_iod *nvme_map_phys_pages(struct nvme_dev *dev, int write,
				       unsigned long addr, unsigned length)
{
	int err;
	struct nvme_phys_iod *iod;

	pr_debug("NVMe: nvme_map_phys_pages(), addr: 0x%08lx, length: %u\n", addr, length);
	...
	err = -ENOMEM;
	iod = __nvme_alloc_phys_iod(1, length, dev, 0, GFP_KERNEL);
	if (!iod)
		goto put_pages;
...
static int nvme_submit_io_kernel(struct nvme_ns *ns, struct nvme_user_io *uio)
{
	...
	pr_debug("NVMe: nvme_submit_io_kernel()\n");

	if (memcpy(&io, uio, sizeof(io)) != &io) {
		pr_debug("NVMe: nvme_submit_io_kernel() memcpy to &io failed\n");
		return -EFAULT;
	}
	length = (io.nblocks + 1) << ns->lba_shift;
	meta_len = (io.nblocks + 1) * ns->ms;
	...
	if (meta_len) {
		...
		if (write) {
			if (memcpy(meta, metadata, meta_len) != meta) {
				pr_debug("NVMe: nvme_submit_io_kernel() memcpy to meta failed\n");
				status = -EFAULT;
				goto unmap;
			}
		}
	...
	if (meta) {
		if (status == NVME_SC_SUCCESS && !write) {
			if (memcpy(metadata, meta, meta_len) != metadata) {
				pr_debug("NVMe: nvme_submit_io_kernel() memcpy to metadata failed\n");
				status = -EFAULT;
			}
		}
		dma_free_coherent(&dev->pci_dev->dev, meta_len, meta, meta_dma);
                                      ...
                                      struct nvme_user_io *uio)
{
	...
	switch (io.opcode) {
		...
		case nvme_cmd_read:
		case nvme_cmd_compare:
			iod = nvme_map_phys_pages(dev, write, io.addr, length);
			break;
		default:
			return -EINVAL;
	}
	...
	prp_len = nvme_setup_phys_prps(dev, iod, length, GFP_KERNEL, io.addr);
	if (length != prp_len) {
		status = -ENOMEM;
		goto unmap;
	}
							...
							unsigned long arg)
{
	...
	pr_debug("NVMe: nvme_ioctl(), cmd: %u\n", cmd);
...
static int nvme_kthread(void *data)
{
	...
	pr_debug("NVMe: nvme_kthread()\n");

	while (!kthread_should_stop()) {

//		pr_debug("NVMe: nvme_kthread() loop\n");
	...
	pr_debug("NVMe: nvme_kthread() done\n");
...
static void nvme_alloc_ns(struct nvme_dev *dev, unsigned nsid)
{
	...
	pr_debug("NVMe: nvme_alloc_ns(), nsid: %u\n", nsid);
...
//		blk_queue_flush(ns->queue, REQ_OP_FLUSH | REQ_FUA);
...
static int nvme_setup_io_queues(struct nvme_dev *dev)
{
	...
	pr_debug("NVMe: nvme_setup_io_queues()\n");
...
static void nvme_async_probe(struct work_struct *work)
{
	...
	pr_debug("NVMe: nvme_async_probe()\n");
