...
//	char *buffer = memalign(4096, BUFFER_LENGTH);
	...
	lseek(fd, 0, SEEK_SET);
	read(fd, in_buf, OCM_SIZE); // Read actual data from device
	memcpy(out_buf, ocm, OCM_SIZE); // Copy OCM to buffer
	...
	infd = open("ocmreadtest-in", O_WRONLY | O_CREAT | O_TRUNC, S_IWUSR | S_IRUSR);
	if (infd < 0) {
	...
	outfd = open("ocmreadtest-out", O_WRONLY | O_CREAT | O_TRUNC, S_IWUSR | S_IRUSR);
	if (outfd < 0) {
		perror("Failed to open output file 1");
		return errno;
	}
